 ...
 * @param txQueueEntries size of the queue for recieving TSI requests
 * @param rxQueueEntries size of the queue for sending TSI responses
...
case class FESVRParams(
  ...
  tlAddressSet: AddressSet = AddressSet(0, 0xFFFF),
  tlRegionType: RegionType.T = RegionType.UNCACHED,
  tlNumTransactions: Int = 8,
  tlLineBytes: Int = 64,
 ...
 */
class FESVRWidgetIO(w: Int) extends Bundle {
...
trait TLFESVRMMIOModule extends HasRegMap {
  ...
  regmap(
    FESVRWidgetCtrlRegs.txQueueOffset -> Seq(RegField.r(p(PeripheryFESVRKey).mmioRegWidth, txQueue.io.enq)),
    FESVRWidgetCtrlRegs.rxQueueOffset -> Seq(RegField.w(p(PeripheryFESVRKey).mmioRegWidth, rxQueue.io.deq))
  )
}
  ...
  extends LazyModule
{
  ...
  val mmio = LazyModule(new TLFESVRMMIO(beatBytes))
  ...
  extTLNode := TLBuffer() := serdes.clientNode
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    mmioMod.io.serial.out <> Queue(adapterMod.io.serial.out)
    adapterMod.io.serial.in <> Queue(mmioMod.io.serial.in)
