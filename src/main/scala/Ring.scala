...
case class RingSystemBusParams(params: SystemBusParams)
  ...
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper])(implicit p: Parameters): SystemBus = {
    val ring = LazyModule(new RingSystemBus(params))
...
class RingSystemBus(params: SystemBusParams)
    (implicit p: Parameters) extends SystemBus(params) {
  val system_bus_ring = LazyModule(new TLRingNetwork())
